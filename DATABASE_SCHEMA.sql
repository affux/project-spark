...
-- 
-- CHANGELOG v2.1.0:
-- - Added site_title, site_favicon_url, site_logo_url to default settings
-- - Added email notification settings (resend_api_key, sender_email, etc.)
-- - Added currency_symbol, footer_text, contact_email, contact_phone
-- - Fixed wallet_transactions type constraint
-- =============================================
...
-- Indian names table (for random name generation - MUST BE CREATED FIRST)
...
-- Storefront products table (MUST BE CREATED BEFORE product_reviews)
...
-- Product reviews table (now storefront_products exists)
...
-- Order customer names table (now indian_names exists)
CREATE TABLE public.order_customer_names (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE public.postpaid_transactions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL,
  order_id UUID REFERENCES public.orders(id),
  amount NUMERIC NOT NULL,
...
-- Chat messages table (user-admin support chat)
...
-- Chat customer names table (now indian_names exists)
CREATE TABLE public.chat_customer_names (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID NOT NULL UNIQUE,
  indian_name_id UUID NOT NULL REFERENCES public.indian_names(id),
  assigned_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
...
CREATE TABLE public.order_chat_quick_replies (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  title TEXT NOT NULL,
  message TEXT NOT NULL,
...
CREATE TABLE public.order_chat_audit_logs (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  order_id UUID NOT NULL REFERENCES public.orders(id) ON DELETE CASCADE,
...
CREATE TABLE public.dashboard_message_targets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE public.popup_message_targets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE public.popup_acknowledgments (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  message_id UUID NOT NULL REFERENCES public.popup_messages(id) ON DELETE CASCADE,
  user_id UUID NOT NULL,
...
CREATE TABLE public.user_rank_reference (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_profiles_postpaid_enabled ON public.profiles(postpaid_enabled);
...
CREATE INDEX idx_orders_dropshipper_user_id ON public.orders(dropshipper_user_id);
...
CREATE INDEX idx_postpaid_transactions_user_id ON public.postpaid_transactions(user_id);
CREATE INDEX idx_postpaid_transactions_order_id ON public.postpaid_transactions(order_id);
...
CREATE INDEX idx_chat_reassignment_logs_session_id ON public.chat_reassignment_logs(chat_session_id);
...
CREATE INDEX idx_email_mfa_codes_user_id ON public.email_mfa_codes(user_id);
CREATE INDEX idx_trusted_devices_user_id ON public.trusted_devices(user_id);
CREATE INDEX idx_support_agent_presence_user_id ON public.support_agent_presence(user_id);
...
CREATE INDEX idx_proof_of_work_user_id ON public.proof_of_work(user_id);
CREATE INDEX idx_proof_of_work_status ON public.proof_of_work(status);
CREATE INDEX idx_work_types_category ON public.work_types(category);
CREATE INDEX idx_user_notifications_user_id ON public.user_notifications(user_id);
CREATE INDEX idx_user_notifications_is_read ON public.user_notifications(is_read);
CREATE INDEX idx_custom_payment_methods_type ON public.custom_payment_methods(method_type);
CREATE INDEX idx_dashboard_messages_enabled ON public.dashboard_messages(is_enabled);
CREATE INDEX idx_dashboard_message_targets_message_id ON public.dashboard_message_targets(message_id);
CREATE INDEX idx_dashboard_message_targets_user_id ON public.dashboard_message_targets(user_id);
CREATE INDEX idx_popup_messages_enabled ON public.popup_messages(is_enabled);
CREATE INDEX idx_popup_message_targets_message_id ON public.popup_message_targets(message_id);
CREATE INDEX idx_popup_message_targets_user_id ON public.popup_message_targets(user_id);
CREATE INDEX idx_popup_acknowledgments_message_id ON public.popup_acknowledgments(message_id);
CREATE INDEX idx_popup_acknowledgments_user_id ON public.popup_acknowledgments(user_id);
CREATE INDEX idx_top_dropshippers_rank ON public.top_dropshippers(rank_position);
CREATE INDEX idx_top_dropshippers_active ON public.top_dropshippers(is_active);
CREATE INDEX idx_user_rank_reference_user_id ON public.user_rank_reference(user_id);
...
BEGIN
  SELECT user_id INTO selected_agent_id
  FROM public.support_agent_presence
  WHERE is_online = true
    AND (_exclude_agent_id IS NULL OR user_id != _exclude_agent_id)
  ORDER BY active_chat_count ASC, last_seen_at DESC
  LIMIT 1;
...
-- Create audit log entry (with admin authorization check)
...
DECLARE
  log_id uuid;
BEGIN
  -- SECURITY: Verify caller is admin before allowing audit log creation
...
COMMENT ON FUNCTION public.create_audit_log(TEXT, UUID, UUID, TEXT, JSONB, JSONB, JSONB, TEXT, UUID) IS 'Creates audit log entries. SECURITY: Only admins can call this function - enforced by has_role check.';
...
AS $$
BEGIN
  DELETE FROM public.email_mfa_codes WHERE expires_at < now();
...
CREATE TRIGGER update_order_chat_quick_replies_updated_at
  BEFORE UPDATE ON public.order_chat_quick_replies
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER update_work_type_categories_updated_at
...
CREATE TRIGGER update_top_dropshippers_updated_at
  BEFORE UPDATE ON public.top_dropshippers
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

CREATE TRIGGER update_user_rank_reference_updated_at
  BEFORE UPDATE ON public.user_rank_reference
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();
...
-- Wallet credit on order completion trigger
CREATE TRIGGER credit_wallet_on_order_completed_trigger
...
CREATE POLICY "Public can view product media" ON public.product_media
  FOR SELECT USING (EXISTS (
    SELECT 1 FROM products p WHERE p.id = product_media.product_id AND p.is_active = true
  ));
...
CREATE POLICY "Users can submit reviews with validation" ON public.product_reviews
...
CREATE POLICY "Users can view their own orders" ON public.orders
  FOR SELECT USING (auth.uid() = dropshipper_user_id);
...
CREATE POLICY "Users can create orders for their products" ON public.orders
  FOR INSERT WITH CHECK (
    EXISTS (
      ...
      AND sp.user_id = orders.dropshipper_user_id
    )
  );

CREATE POLICY "Users can update their own orders" ON public.orders
  FOR UPDATE USING (auth.uid() = dropshipper_user_id) WITH CHECK (auth.uid() = dropshipper_user_id);
...
CREATE POLICY "Users can insert status history for their orders" ON public.order_status_history
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM orders o
      WHERE o.id = order_status_history.order_id
...
CREATE POLICY "Users can view order customer names" ON public.order_customer_names
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM orders o
      WHERE o.id = order_customer_names.order_id
      AND o.dropshipper_user_id = auth.uid()
    )
  );

CREATE POLICY "Users can insert order customer names for their orders" ON public.order_customer_names
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM orders o
      WHERE o.id = order_customer_names.order_id
...
CREATE POLICY "Admins can manage order customer names" ON public.order_customer_names
  FOR ALL USING (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view their own postpaid transactions" ON public.postpaid_transactions
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own postpaid transactions" ON public.postpaid_transactions
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Admins can manage all postpaid transactions" ON public.postpaid_transactions
  FOR ALL USING (has_role(auth.uid(), 'admin')) WITH CHECK (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can update their own pending payout requests" ON public.payout_requests
  FOR UPDATE USING (auth.uid() = user_id AND status = 'pending') WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete their own pending payout requests" ON public.payout_requests
...
CREATE POLICY "Users can view their own payout history" ON public.payout_status_history
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "Admins can view all payout history" ON public.payout_status_history
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can insert payout history" ON public.payout_status_history
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view their own ratings" ON public.chat_ratings
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own ratings" ON public.chat_ratings
  FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own chat name" ON public.chat_customer_names
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own chat name" ON public.chat_customer_names
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Admins can view all chat names" ON public.chat_customer_names
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can insert chat names" ON public.chat_customer_names
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can update chat names" ON public.chat_customer_names
  FOR UPDATE USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can delete chat names" ON public.chat_customer_names
  FOR DELETE USING (has_role(auth.uid(), 'admin'));

-- Chat sessions policies
CREATE POLICY "Users can view their own session" ON public.chat_sessions
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can upsert their own session" ON public.chat_sessions
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own session" ON public.chat_sessions
  FOR UPDATE USING (auth.uid() = user_id);
...
CREATE POLICY "Admins can view all reassignment logs" ON public.chat_reassignment_logs
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can insert reassignment logs" ON public.chat_reassignment_logs
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));

-- Indian names policies
CREATE POLICY "Anyone can read active indian names" ON public.indian_names
  FOR SELECT USING (is_active = true);

CREATE POLICY "Admins can manage indian names" ON public.indian_names
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Order chat messages policies
CREATE POLICY "Users can view order chat messages" ON public.order_chat_messages
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM orders o
...
CREATE POLICY "Users can send order chat messages" ON public.order_chat_messages
  FOR INSERT WITH CHECK (
    sender_type = 'user' 
...
CREATE POLICY "Users can mark messages as read" ON public.order_chat_messages
  FOR UPDATE USING (
...
CREATE POLICY "Admins can view all order chat messages" ON public.order_chat_messages
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can send customer messages" ON public.order_chat_messages
  FOR INSERT WITH CHECK (
...
CREATE POLICY "Admins can update order chat messages" ON public.order_chat_messages
  FOR UPDATE USING (has_role(auth.uid(), 'admin'));

-- Order chat quick replies policies
CREATE POLICY "Users can view active quick replies" ON public.order_chat_quick_replies
  FOR SELECT USING (is_active = true);

CREATE POLICY "Admins can manage quick replies" ON public.order_chat_quick_replies
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Order chat audit logs policies
CREATE POLICY "Admins can view order chat audit logs" ON public.order_chat_audit_logs
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can insert order chat audit logs" ON public.order_chat_audit_logs
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view their own logout events" ON public.force_logout_events
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Admins can insert force logout events" ON public.force_logout_events
  FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users can view their own email MFA codes" ON public.email_mfa_codes
  FOR SELECT USING (auth.uid() = user_id);
...
CREATE POLICY "Admins can manage agent presence" ON public.support_agent_presence
  FOR ALL USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Users can view online agents" ON public.support_agent_presence
...
CREATE POLICY "Admins can view all crypto payments" ON public.crypto_payments
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins can update crypto payments" ON public.crypto_payments
  FOR UPDATE USING (has_role(auth.uid(), 'admin'));

-- Proof of work policies
CREATE POLICY "Users can view their own proofs" ON public.proof_of_work
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create their own proofs" ON public.proof_of_work
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their pending proofs" ON public.proof_of_work
  FOR UPDATE USING (auth.uid() = user_id AND status = 'pending');

CREATE POLICY "Admins can manage all proofs" ON public.proof_of_work
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Work types policies
CREATE POLICY "Authenticated users can view active work types" ON public.work_types
  FOR SELECT USING (is_active = true);

CREATE POLICY "Admins can manage work types" ON public.work_types
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Work type categories policies
CREATE POLICY "Authenticated users can view active categories" ON public.work_type_categories
  FOR SELECT USING (is_active = true);

CREATE POLICY "Admins can manage work type categories" ON public.work_type_categories
  FOR ALL USING (has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Anyone can read enabled payment methods" ON public.custom_payment_methods
  FOR SELECT USING (is_enabled = true);

CREATE POLICY "Admins can manage payment methods" ON public.custom_payment_methods
  FOR ALL USING (EXISTS (
    SELECT 1 FROM user_roles
    WHERE user_roles.user_id = auth.uid() AND user_roles.role = 'admin'
  ));
...
CREATE POLICY "Users can view enabled messages" ON public.dashboard_messages
  FOR SELECT USING (
    is_enabled = true 
...
CREATE POLICY "Admins can manage dashboard messages" ON public.dashboard_messages
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Dashboard message targets policies
CREATE POLICY "Users can view their own message targets" ON public.dashboard_message_targets
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Admins can manage message targets" ON public.dashboard_message_targets
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- Popup messages policies
CREATE POLICY "Users can view their targeted popup messages" ON public.popup_messages
  FOR SELECT USING (
    is_enabled = true
    AND (
      EXISTS (
      ...
      OR (target_type = 'specific' AND EXISTS (
        SELECT 1 FROM popup_message_targets
        WHERE popup_message_targets.message_id = popup_messages.id
        AND popup_message_targets.user_id = auth.uid()
      ))
      OR (target_type = 'role' AND target_roles IS NOT NULL AND EXISTS (
        SELECT 1 FROM user_roles
        WHERE user_roles.user_id = auth.uid()
        AND user_roles.role::text = ANY(popup_messages.target_roles)
      ))
    )
  );

CREATE POLICY "Admins can manage popup messages" ON public.popup_messages
  FOR ALL USING (has_role(auth.uid(), 'admin')) WITH CHECK (has_role(auth.uid(), 'admin'));

-- Popup message targets policies
CREATE POLICY "Users can view their own popup targets" ON public.popup_message_targets
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Admins can manage popup message targets" ON public.popup_message_targets
  FOR ALL USING (has_role(auth.uid(), 'admin')) WITH CHECK (has_role(auth.uid(), 'admin'));

-- Popup acknowledgments policies
CREATE POLICY "Users can manage their own acknowledgments" ON public.popup_acknowledgments
  FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

CREATE POLICY "Admins can view all acknowledgments" ON public.popup_acknowledgments
  FOR SELECT USING (has_role(auth.uid(), 'admin'));

-- Top dropshippers policies (admin only, use view for public access)
CREATE POLICY "Admins can manage top dropshippers" ON public.top_dropshippers
  FOR ALL USING (has_role(auth.uid(), 'admin'));

-- User rank reference policies
CREATE POLICY "Admins can manage user rank reference" ON public.user_rank_reference
  FOR ALL USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Users can view their own rank" ON public.user_rank_reference
  FOR SELECT USING (auth.uid() = user_id);
...
-- Top dropshippers public view (masks sensitive data)
CREATE OR REPLACE VIEW public.top_dropshippers_public AS
SELECT 
  id,
  display_name,
  rank_position,
  orders_count,
  NULL::numeric as earnings_amount,
  badge_title,
  is_active,
  NULL::uuid as user_id,
  created_at,
  updated_at
FROM public.top_dropshippers
...
BEGIN
  ...
  INTO email_attempts, last_attempt
  FROM login_attempts
...
CREATE OR REPLACE FUNCTION public.record_login_attempt(
  _email text,
  _ip_address text DEFAULT NULL,
  _was_successful boolean DEFAULT false
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 2. branding (public)
-- 3. videos (public)
-- 4. payout-documents (private)
-- 5. payment-proofs (private)
-- 6. proof-of-work (private)
-- 7. product-media (public)
