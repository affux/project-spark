...
-- Add video_tutorials and user_dashboard_video_url to the Restricted policy
...
USING (has_role(auth.uid(), 'admin'::app_role) OR (key = ANY (ARRAY[
  ...
  -- Video & tutorial settings (added for user access)
  'video_tutorials'::text, 'user_dashboard_video_url'::text, 'landing_video_url'::text,
  'faq_items'::text
])));
