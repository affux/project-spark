...
BEGIN
  ...
  -- Check if there was an order_payment deduction (regardless of payment_type field)
  IF NEW.status = 'pending_payment'::public.order_status 
    ...
    -- Check if there was a wallet payment deduction for this order (order_payment type with negative amount)
    ...
    -- Check if refund was already issued and there's a payment to refund
    IF wallet_payment_amount > 0 AND NOT EXISTS (
      ...
      -- Also clear paid_at and payment_proof_url since order is back to pending
      NEW.paid_at := NULL;
