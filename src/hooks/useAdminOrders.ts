...
export const useAdminOrders = () => {
  ...
  // Create order mutation for admin with encryption
  const createOrderMutation = useMutation({
    mutationFn: async (orderData: {
      ...
      // Encrypt sensitive customer data via edge function
      let encryptedPhone = orderData.customer_phone;
      let encryptedAddress = orderData.customer_address;

      try {
        const { data: encryptedData, error: encryptError } = await supabase.functions.invoke('encrypt-order-data', {
          body: {
        ...
        if (!encryptError && encryptedData?.success) {
          encryptedPhone = encryptedData.data.customer_phone;
          encryptedAddress = encryptedData.data.customer_address;
        }
      } catch (encryptionError) {
        console.warn('Encryption failed, storing unencrypted:', encryptionError);
  ...
  // Bulk create orders mutation for admin with encryption
  const bulkCreateOrderMutation = useMutation({
    mutationFn: async (data: {
      ...
      // Encrypt sensitive data for each order
      const ordersToInsert = await Promise.all(data.orders.map(async (order) => {
        let encryptedPhone = order.customer_phone;
        let encryptedAddress = order.customer_address;
        ...
        try {
          ...
          console.warn('Encryption failed for bulk order, storing unencrypted:', encryptionError);
        }
